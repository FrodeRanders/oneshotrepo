...
class TimeValue extends Value<Timestamp> {
    private final static String columnName = "timeval";
    ...
    public void set(ArrayList<Timestamp> values) throws AttributeTypeException {
    ...
    /* package protected */ void store(
        ...
        try {
            if (!values.isEmpty()) {
                try (PreparedStatement pStmt = conn.prepareStatement(ctx.getStatements().valueDateInsert())) {
                    int index = 0;
                    for (Timestamp date : values) {
                        int i = 0;
