...
final class IntegerValue extends Value<Integer> {
    private final static String columnName = "intval";
    ...
    /* package protected */ void store(
        ...
        try {
            if (!values.isEmpty()) {
                try (PreparedStatement pStmt = conn.prepareStatement(ctx.getStatements().valueIntegerInsert())) {
                    int index = 0;
